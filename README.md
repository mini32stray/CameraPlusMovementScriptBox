...
CameraPlusの曲専用のMovement Script (カメラスクリプト) の選択をゲーム内からできるようにするMODです。
CameraPlusの動作を勝手に変更します。

## 概要

- 曲ごとのカメラスクリプトは、本来は曲フォルダに入れる必要がありますが、このMODでは **専用のフォルダ** に入れます。
- 曲ごとのカメラスクリプトは、本来は入れたら毎回必ず実行される仕様ですが、このMODではスクリプトの **実行の有無を選択** できます。
- 曲ごとのカメラスクリプトは、本来は1曲に対し複数入れることはできませんが、このMODでは **複数のスクリプトから選択** できます。
- 実行中のカメラスクリプトの **作者をパネルに表示** できます。(`StandaloneBeatmapInformation` 対応：v0.3.1以降)
- このMODで入れているカメラスクリプトの対象マップを抽出し、 **プレイリスト化** できます。 (v0.4.1以降)
...
## 使い方
- CameraPlusの設定で `Song specific script` は **有効に** してください。

- カメラスクリプトは、後述のルールに従って **ファイル名をつけて** `UserData\CameraPlusMovementScriptBox` に置いてください。
...
- 以下の注意点に気を付けてね。
  ...
  - (Optional) プレイリストはサブフォルダ内にも出力できますが、そのためには設定ファイルを直接編集して設定する必要があります。
